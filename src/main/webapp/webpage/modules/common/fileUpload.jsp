...
<head>
	...
	<script type="text/javascript">
        var basedir = ""; //路径
        var weburl = "";  //图片web访问地址，outSidePath为true时使用
        var outSidePath = "${outSidePath}";
        if(outSidePath=="true"){
            weburl = "${vehicleImages}";
            basedir = "${basedir}";
        }
        // 添加全局站点信息
        var BASE_URL = '/webuploader';
        var uploadUrl = '${ctx}/sys/file/webupload/upload?uploadPath=${uploadPath}&outSidePath=${outSidePath}';
        ...
        function getUploadFileValues() {
            ...
            for(var i=0; i < list.length; i++){
                ...
                var valtemp = $(list[i]).attr("data-url");
                if(outSidePath=="true"){
                    var index = valtemp.indexOf("vehicleImages");
                    if(index!=-1){
                        valtemp = valtemp.substring(index+13);
					}
				}
